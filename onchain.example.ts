...
async function run() {
  // 1) Request payment quote from server for 1 USDC
  const quote = await fetch("http://localhost:4000/api/payment?amount=1");
  ...
  if (!recipientAccountExists) {
    // Use the recipient wallet address from the payment requirements
    const recipientWallet = new PublicKey(q.payment.recipientWallet);
  ...
  const paid = await fetch("http://localhost:4000/api/payment", {
    headers: {
